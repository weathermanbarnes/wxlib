...
from metopen_pt import metopen_pt
...
# globally useful
f, oro = metopen_pt('ei.ans.orog.subsamp.filtx120y160', 'z')
oro = np.squeeze(oro)
oro = concat1(oro) #adds a lon
s   = oro.shape
lat = np.tile(f.variables['latitude'][c.std_slice[1]], (s[1],1)).T
lon = np.tile(f.variables['longitude'][c.std_slice[2]], (s[0],1))
lon = concat1(lon)
lon[:,-1] += 360
f.close()
...
scale_u_cent = np.arange(-60,61,10)
...
mapcase={'w':wmap,'np':npmap,'sp':spmap,'Green':Greenmap,'NA':NAmap,'NP':NPmap, 'Sibir':Sibirmap,'Aus':Ausmap}

def mapfunc(maptype):
	return mapcase[maptype]()
...
def LevelColormap(levels, cmap=None):
	...
	return mpl.colors.LinearSegmentedColormap('%s_levels' % cmap.name, cdict, 256)
...
# prepares basemap for subplot
def sub_conf_con(m,dat,pv, x,y, mark=None, cmap=None, scalef=10, scale=10,overlays=[], title='', ticks=None):
	dat = concat1(dat)
	pv = concat1(pv)
	#dat[:5,:] = np.nan
	#dat[-5:,:] = np.nan
	m.drawcoastlines(linewidth=0.5)
	m.contour(x,y, pv, scale, colors='k', zorder=2,linewidths=1)
	cfo=m.contourf(x,y, dat, scalef, zorder=1, cmap=cmap, extend='both')
	m.drawparallels(range(-80,81,10))
...
def map_conf_con(m,dat,pv,plev=None, mark=None, cmap=None, scalef=10, scale=10,overlays=[], show=True, save='', title='', ticks=None, lon=lon, lat=lat):
	dat = concat1(dat)
	pv = concat1(pv)
	if plev:
	...
	m.drawcoastlines(linewidth=0.5)
	x,y = m(lon,lat)
	m.contour(x,y, pv, scale, colors='k', alpha=0.4, zorder=2,linewidths=2)
	if plev:
		m.contourf(x, y, mask, cmap=_get_grey_cm())
	m.contourf(x, y, dat, scalef, zorder=1, cmap=cmap, extend='both')
	m.drawparallels(range(-80,81,10))
	m.drawmeridians(range(0,360,30))
	plt.colorbar(ticks=ticks,orientation='vertical',format='%.1e')
	for overlay in overlays:
		overlay(m,x,y, zorder=2, mask=mask)
	if mark:
		yidx, xidx = mark
...
def add_barb(m, u, v, mark=None, quiver=False, cmap=None, 
		show=True, save='', title='', thin=3):
	u   = concat1(u)
	v   = concat1(v)
	x,y = m(lon,lat)
	ut,vt,xt,yt = m.transform_vector(u[::-1,:],v[::-1,:],lon[0,:],lat[::-1,0],60,60,returnxy=True)
	if not quiver:
		m.barbs(xt[::thin,::thin], yt[::thin,::thin], ut[::thin,::thin], vt[::thin,::thin], length=6, linewidth=0.5, zorder=3)
	else:
		m.quiver(xt[::thin,::thin], yt[::thin,::thin], ut[::thin,::thin], vt[::thin,::thin], zorder=3)
	if mark:
		yidx, xidx = mark
...
def map_oro_dat(m, dat, plev=None, mark=None, cmap=None, scale=25, overlays=[], show=True, save='', title='', ticks=None, lon=lon, lat=lat):
	...
	m.drawparallels(range(-80,81,10),color='gray')
	m.drawmeridians(range(0,360,30),color='gray')
	plt.colorbar(ticks=ticks,orientation='vertical',format='%.1e',size="5%", pad='2%')
	for overlay in overlays:
		overlay(m,x,y, zorder=2, mask=mask)
	if mark:
		yidx, xidx = mark
