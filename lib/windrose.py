...
__version__ = '1.4-localmods'
__author__ = 'Lionel Roubeyrie'
__mail__ = 'lionel.roubeyrie@gmail.com'
__license__ = 'CeCILL-B'

# 
# Modified by Clemens Spensberger 
# (1) included in the dynlib plotting library.
# (2) extended to (also) plot already binned data 
# (3) extended to plot half-roses for deformation angles
# (4) updated to recent matplotlib following https://github.com/matplotlib/matplotlib/issues/11911
#
...
class WindroseAxes(PolarAxes):
    ...
    def _init_plot(self, dir, var, **kwargs):
        ...
        # Halfing the histogram
        half = kwargs.get('halfplot', False)

        #Number of sectors
        nsector = kwargs.pop('nsector', None)
        prebinned = kwargs.pop('prebinned', None)
        if type(prebinned) == np.ndarray: 
            nsector = len(prebinned)-1
            if half: nsector *= 2
        if nsector is None:
            nsector = 16
        ...
        if not type(prebinned) == np.ndarray:
            self._info['dir'], self._info['bins'], self._info['table'] = histogram(dir, var, bins, nsector, normed, blowto)
        else: 
            if normed:
                dir = dir*100/dir.sum()
            if half: 
                prebinned = np.append(prebinned[:-1], prebinned + np.pi)
                dir = np.append(dir, dir)
            
            angles = prebinned[:-1] + np.pi/nsector
            self._info['dir'] = prebinned*(180.0/np.pi)
            self._info['bins'] = np.array([0.0, np.inf])
            self._info['table'] = dir.reshape((1,len(dir)) )
    ...
    def _halfmask(self):
        if not self.halfmasked:
            PolarAxes.bar(self, [np.pi/2.0, ], [self.get_rmax(), ], [np.pi, ], 
                    bottom=0.0, facecolor=(0.9,0.9,0.9), alpha=0.8)
            self.halfmasked = True
    ...
    def contour(self, dir, var, **kwargs):
        ...
        * cmap : a cm Colormap instance from matplotlib.cm.
        ...
        vals = np.hstack((self._info['table'], np.reshape(self._info['table'][:,0],
...
# Get current axes, if they are WindroseAxes; otherwise generate new current WindroseAxes
def gca_or_mca():
    ax = plt.gca()
    if isinstance(ax, WindroseAxes):
        return ax
