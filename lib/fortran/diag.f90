...
contains
  ...
  subroutine slope(slope, dzdx,dzdy,dzdth, nx,ny,nz,nt, z, dx,dy,dth)
    use consts !, only: nan
    ...
    !
    ! Gradient and slope
    call grad_3d(dzdx,dzdy,dzdth nx,ny,nz, z, dx,dy,dth)
    ...
    where (slope > thres_max_slope)
        slope = nan
    end where
    !
    where (dzdth .gt. thres_max_dzdth)
        dzdth = nan
    end where
    !
end module
